...
<form action="home/myaddress" method="post" name="myform">
        {{ csrf_field() }}
        {{  method_field('DELETE') }}
</form>
<!-- 收货地址 -->
            <div class="user_address" id="outerDiv" style="width: 1032px;
border: 1px solid #d9d8d6;

color: #000;
float: right;">
            <h3 style="height: 38px;line-height: 38px;padding-left: 23px;border-bottom: 1px solid #d9d8d6;font-size: 15px;font-weight: normal;background: #eee;
">我的收货地址</h3>
            <div class="address_con" style="padding: 0 13px 58px 13px;min-height: 500px;position: relative;">
                <p class="address_tip" style="color: #6f7170;padding: 15px 0 0 10px;">设置好轻松购信息，就可以在商品页面直接下单 ，购物从此变得简单快乐!</p>
                <div class="address_add clearfix" style="padding: 22px 0 4px 0;">
                    <a href="/home/address/create" style="float: left;	display: block;height: 24px;line-height: 26px;border: 1px solid #d9d8d6;border-radius: 5px;padding: 0 12px;font-size: 13px;">新增收货地址</a>
                </div>
                <input type="hidden" id="isDefault"/>
...
<form action="/home" method="post">
    ...
    <div class="address_list">
        <!-- <div class="address_title">aaa&nbsp;北京北京北京北京昌平区</div> -->
             @foreach($list as $v)
             <table style="border-top: 1px solid #d9d8d6;font-size: 15px;font-weight: normal;" class="aaa">
                <div style="float:left;" class="bbb">
                    <tr><td>收货人&nbsp;&nbsp;&nbsp;：{{ $v->Address_consignee }}</td></tr>
                    <tr class="ccc"><td>收货地址：<span>{{ $v->Address_detail }}</span></td></tr>
                    <tr><td>收货手机：{{ $v->Address_consignee_phone }}</td></tr>
                    <tr><td>收货邮箱：{{ $v->Address_email }}</td></tr>   
                </div> 
             </table>
             <a href="/home/aaddress/{{ $v->Address_detail }}" class="zz">设为默认地址</a>
             <a href="/home/myaddress/{{ $v->Address_id }}/edit">编辑</a>
             <a href="javascript:doDel({{ $v->Address_id }})">删除</a> 
             @endforeach           
    </div>
