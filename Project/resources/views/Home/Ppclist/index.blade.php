...
<!--                <span class="searchKeyRed"style="display: none;"><em class="globalSearchKey"></em><i class="del"></i></span>-->
        ...
        <ul>
        	<li style="display:inline;"><a href="http://cplenovo.com/ppclist?@if(!empty($where['Goods_name']))Goods_name={{ $where['Goods_name'] }} &@endif @if(!empty($where['Goods_price']))Goods_price[]={{ $where['Goods_price'][0] }}&Goods_price[]={{ $where['Goods_price'][1] }} &@endif page=1"><<</a></li>
        	@for($i = 0;$i < $count;$i++)
        	<li style="display:inline;"><a href="http://cplenovo.com/ppclist?@if(!empty($where['Goods_name']))Goods_name={{ $where['Goods_name'] }} &@endif @if(!empty($where['Goods_price']))Goods_price[]={{ $where['Goods_price'][0] }}&Goods_price[]={{ $where['Goods_price'][1] }}&@endif page={{ $i+1 }}">{{ $i+1 }}</a></li>
        	@endfor
        	
        	<li style="display:inline;"><a href="http://cplenovo.com/ppclist?@if(!empty($where['Goods_name']))Goods_name={{ $where['Goods_name'] }} &@endif @if(!empty($where['Goods_price']))Goods_price[]={{ $where['Goods_price'][0] }}&Goods_price[]={{ $where['Goods_price'][1] }}&@endif page={{ $count }}">>></a></li>
        </ul>
